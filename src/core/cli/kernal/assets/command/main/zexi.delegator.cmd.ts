...
import type ZexiApp from "../../app/app";
...
export class ZexiDelegatorCommand {
    readonly #_controller: ZexiCommandController<'dynamic'>;

    constructor(name: string, delegatedTo: ZexiApp) {
        if (!delegatedTo) {
            throw new TypeError('Expected delegatedTo to be a ZexiApp instance');
        }

        const cmd = new CLICommand(name, 'dynamic', delegatedTo);
    ...
    _internal = {
         ...
         */
        accessCMD: (via?: symbol): CLICommand<'dynamic'> | null => {
...
export default ZexiDelegatorCommand;